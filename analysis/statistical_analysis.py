...
class Statistical_Analysis:
        ...
        if not self.table_exists(result_table) or replace:
            ...
            while True:
                ...
                sql_insert_batch = f"""
                    ...
                    INSERT INTO {result_table} (h3_index, population, rent_sqm, regiostar_17, objart, objart_txt, geom)
                    SELECT cte.h3_index,
    ...
    def combine_additional_data_and_heatmaps(self, poi_category, replace=False):
        ...
        """
        for mode, resolution in [("bicycle", 10), ("walking", 9)]:
            ...
            if not self.table_exists(result_table) or replace:
                ...
                WITH ranked_data AS (
                    SELECT
                        had.*,
                        hbcap.accessibility AS accessibility_{mode}_closest_average,
                        hbccgp.accessibility AS accessibility_{mode}_combined_gaussian,
                        hbgp.accessibility AS accessibility_{mode}_gaussian,
...
if __name__ == "__main__":
    try:
        ...
        # combine heatmaps and additional data -> loop over POI categories from confing -> insert into new function
        for poi_category in poi_dicts:
            analysis.combine_additional_data_and_heatmaps(poi_category)
